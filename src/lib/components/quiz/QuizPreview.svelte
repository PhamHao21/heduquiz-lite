<!-- src/lib/components/quiz/QuizPreview.svelte -->
<script lang="ts">
  ...
  {:else}
    {#each questions as q (q.position)}
      <div class="rounded-xl border bg-white p-4 shadow-sm dark:bg-slate-900
        {q.error ? 'border-red-300 dark:border-red-800' : 'border-slate-200 dark:border-slate-700'}">
        <div class="mb-2 flex flex-wrap items-center gap-2">
          <span class="rounded-full bg-indigo-100 px-2.5 py-0.5 text-xs font-semibold text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300">
            {q.position}
          </span>
          {#if q.multi}
            <span class="rounded-full bg-amber-100 px-2 py-0.5 text-xs text-amber-700 dark:bg-amber-950 dark:text-amber-300">Nhiều đáp án</span>
          {/if}
          {#if q.error}
            <span class="rounded-full bg-red-100 px-2 py-0.5 text-xs text-red-600 dark:bg-red-950 dark:text-red-400">{ERROR_LABELS[q.error]}</span>
          {/if}
        </div>
        <p class="mb-3 whitespace-pre-line font-medium text-slate-800 dark:text-slate-100">{q.content || '(Nội dung trống)'}</p>
        {#if q.options.length > 0}
          <ul class="space-y-1.5">
            {#each q.options as opt (opt.label || opt.text)}
              <li class="flex items-start gap-2 rounded-lg px-3 py-2 text-sm
                {opt.is_correct ? 'bg-emerald-50 text-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-300' : 'bg-slate-50 text-slate-700 dark:bg-slate-800 dark:text-slate-300'}">
                {#if opt.label}
